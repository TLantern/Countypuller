...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Check if user has permission for Hillsborough NH
    if (userExists.userType !== 'HILLSBOROUGH_NH') {
      return NextResponse.json({ 
        error: 'Access denied - user not authorized for Hillsborough NH pulls',
        debug: { userType: userExists.userType, required: 'HILLSBOROUGH_NH' }
      }, { status: 403 });
    }
    
    console.log('✅ UserId verified in database with HILLSBOROUGH_NH access:', userExists.email);
  ...
  try {
    ...
    const dynamicLimit = 10; // Fixed to 10 records per pull
    ...
    return NextResponse.json({ 
      ...
      job_id: job.id,
      status: job.status,
      message: 'Hillsborough NH scraping job queued successfully. Use GET /api/pull-hillsborough-nh/{job_id} to check status.'
    });
  } catch (error) {
    console.error('Failed to create Hillsborough NH scraping job:', error);
    return NextResponse.json({ 
      success: false, 
...
export async function GET(req: NextRequest) {
  ...
  try {
    const job = await prisma.scraping_job.findFirst({
      where: { id: jobId, userId, job_type: 'HILLSBOROUGH_NH_PULL' }
    });
    if (!job) {
      return NextResponse.json({ 
        success: false, 
        error: 'Hillsborough NH job not found' 
      }, { status: 404 });
    }
    return NextResponse.json({ 
      success: true, 
    ...
    console.error('Failed to get Hillsborough NH job status:', error);
    return NextResponse.json({ 
      success: false, 
