...
export async function POST(req: NextRequest) {
  ...
  console.log('=== BREVARD FL SESSION DEBUG ===');
  ...
  try {
    ...
    // Check if user has permission for Brevard FL
    if (userExists.userType !== 'BREVARD_FL') {
      return NextResponse.json({ 
        error: 'Access denied - user not authorized for Brevard FL pulls',
        debug: { userType: userExists.userType, required: 'BREVARD_FL' }
      }, { status: 403 });
    }
    
    console.log('✅ UserId verified in database with BREVARD_FL access:', userExists.email);
  ...
  try {
    ...
    return NextResponse.json({ 
      ...
      job_id: job.id,
      status: job.status,
      message: 'Brevard FL scraping job queued successfully. Use GET /api/pull-brevard-fl/{job_id} to check status.'
    });
  } catch (error) {
    console.error('Failed to create Brevard FL scraping job:', error);
    return NextResponse.json({ 
      success: false, 
...
export async function GET(req: NextRequest) {
  ...
  try {
    const job = await prisma.scraping_job.findFirst({
      where: { id: jobId, userId, job_type: 'BREVARD_FL_PULL' }
    });
    if (!job) {
      return NextResponse.json({ 
        success: false, 
        error: 'Brevard FL job not found' 
      }, { status: 404 });
    }
    return NextResponse.json({ 
      success: true, 
    ...
    console.error('Failed to get Brevard FL job status:', error);
    return NextResponse.json({ 
      success: false, 
